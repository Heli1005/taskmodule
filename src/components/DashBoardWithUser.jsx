...
const DashBoardWithUser = () => {
    ...
    console.log("taskData,user", taskData, user);
    ...
    return <Box display={'flex'} px={6} w={'full'} className="bg-gradient-to-r from-teal-600 to-slate-200" h={'100vh'} >
        <VStack w="full">
            <Box display="flex" flexWrap="wrap" w="full" gap={6} mt={10} justifyContent={{ base: "center", md: "flex-start" }}>
                {
                    list.map(card => {
                        return <Box w={{ base: "80%", md: "270px" }} p={7} h={'150px'} bg={'white'} rounded={'lg'} shadow={'3px 4px 10px black'}>
                            <VStack w="full">
